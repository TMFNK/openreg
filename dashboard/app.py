...
# Authenticated user section
    ...
    with col1:
        ...
        st.success(f"Welcome *{st.session_state.user_name}* ({role})")
    with col2:
        if st.button("Logout"):
            st.session_state.authenticated = False
            st.session_state.username = None
...
if "FINREP" in available_tabs:
    with tabs[tab_index]:
        ...
        fig = px.bar(
            ...
            x="sector",
            y="exposure_amount",
            color="credit_quality_bucket",
            title="Credit Quality by Sector",
            barmode="stack",
        )
