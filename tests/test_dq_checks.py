...
class TestDataQualityEngine:
    ...
    def test_completeness_check_pass(self, sample_customer_data):
        ...
        completeness = dq_engine._check_completeness(sample_customer_data, 'customer_id')
    ...
    def test_pd_rating_range_check(self, sample_loan_data):
        ...
        dq_engine = DataQualityEngine()
        violations = dq_engine._check_pd_range(sample_loan_data,
                                               min_pd=0.001, max_pd=0.99)
        assert violations == 0
    ...
    def test_pd_rating_out_of_range(self, sample_loan_data):
        ...
        dq_engine = DataQualityEngine()
        violations = dq_engine._check_pd_range(invalid_pd_data,
                                               min_pd=0.001, max_pd=0.99)
        assert violations == 1
    ...
    def test_accounting_balance_check(self):
        ...
        accounts = pd.DataFrame({'customer_id': ['C1', 'C2'], 'account_id': ['A1', 'A2']})
    ...
    @pytest.mark.parametrize("threshold,target_score", [
        (0.98, 0.98),
        (0.95, 0.95),
        (0.99, 0.99)
    ])
...
class TestRegulatoryCalculations:
    ...
    def test_ead_calculation(self, sample_loan_data):
        ...
        assert all(ead >= outstanding for ead, outstanding in zip(ead_values, outstanding_values))
    ...
    def test_pd_lgd_el_relationship(self, sample_loan_data):
        ...
        expected_el = (sample_loan_data['pd_rating'] *
