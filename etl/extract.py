...
class SyntheticBankDataGenerator:
    ...
    def generate_customers(self):
        ...
        for i in tqdm(range(self.num_customers), desc="Customers"):
            customers.append({
                ...
                'country': np.random.choice(['DE', 'FR', 'IT', 'ES', 'NL'], p=[0.3, 0.2, 0.2, 0.15, 0.15]),
                'sector': np.random.choice(['MANUFACTURING', 'REAL_ESTATE', 'FINANCIAL', 'AGRICULTURE', 'SERVICES',
                                           'CONSTRUCTION', 'RETAIL_TRADE', None], p=[0.1, 0.15, 0.1, 0.05, 0.25, 0.1, 0.15, 0.1])
            })
    ...
    def generate_accounts(self):
        ...
        for i in tqdm(range(self.num_accounts), desc="Accounts"):
            accounts.append({
                ...
                'record_source': 'SYNTHETIC',
                'load_datetime': datetime.now(),
                'account_type': np.random.choice(['CURRENT', 'SAVINGS', 'TERM_DEPOSIT'], p=[0.6, 0.3, 0.1]),
    ...
    def generate_loans(self):
        ...
        for i in tqdm(range(self.num_loans), desc="Loans"):
            ...
            loans.append({
                ...
                'sector': np.random.choice(['REAL_ESTATE', 'MANUFACTURING', 'SERVICES', 'CONSTRUCTION'], 
                                         p=[0.4, 0.2, 0.25, 0.15])
            })
    ...
    def generate_gl_entries(self, loans_df, transactions_df):
        ...
        for _, loan in loans_df.iterrows():
            ...
            gl_entries.append({
                'gl_entry_id': f"GL_{hashlib.md5(f"{loan['loan_id']}_DR".encode()).hexdigest()}",
            ...
            # Credit: Cash
            gl_entries.append({
                'gl_entry_id': f"GL_{hashlib.md5(f"{loan['loan_id']}_CR".encode()).hexdigest()}",
                'entry_date': loan['origination_date'],
                'account': '1000_CASH',
                'debit_credit': 'CR',
                'amount': loan['principal_amount'],
