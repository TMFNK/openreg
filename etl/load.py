...
class DataVaultLoader:
    ...
    def run_full_load(self, datasets):
        ...
        self.load_hub(
            datasets["customers"][
                ["customer_hash", "customer_id", "load_datetime", "record_source"]
            ],
            "customer",
        )
        self.load_hub(
            datasets["accounts"][
                ["account_hash", "account_id", "load_datetime", "record_source"]
            ],
            "account",
        )
        self.load_hub(
            datasets["loans"][
                ["loan_hash", "loan_id", "load_datetime", "record_source"]
            ],
            "loan",
        )
        ...
        try:
            ...
            loans_lookup = dict(
                zip(datasets["loans"]["loan_id"], datasets["loans"]["loan_hash"])
            )
            ...
            collateral_df.to_sql(
                "sat_collateral", self.conn, if_exists="append", index=False
            )
        ...
        cust_acc_links, acc_loan_links = transformer.create_links(
            datasets["customers"], datasets["accounts"], datasets["loans"]
        )
        self.load_link(cust_acc_links, "customer_account")
        self.load_link(acc_loan_links, "account_loan")

        # Load GL entries
        datasets["gl_entries"].to_sql(
            "hub_gl_entries", self.conn, if_exists="append", index=False
        )
